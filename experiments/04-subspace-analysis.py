...
MODEL_PATH = "experiments/outputs-03/0-hmm_discrete_model.pt"
...
def process_batches(model, data_iter, num_batches, mode="Train", accumulators=None):
    ...
    for _ in tqdm(range(num_batches), desc=desc):
        ...
        coord_chunks = chunk_batch(theta_bins, SEQ_LEN)
        vel_chunks = chunk_batch(batch["velocity"].to(device), SEQ_LEN)
        belief_chunks = chunk_batch(batch["belief_state"].to(device), SEQ_LEN)
        
        for c_idx in range(len(coord_chunks)):
            inputs = coord_chunks[c_idx]
